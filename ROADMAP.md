...
0. **Reset-password link doesn't reach the reset screen — next major fix.** Clicking the link from the actual reset email lands back on the sign-in page instead of `reset-password.html`, so there's currently no working way to actually change a forgotten password end-to-end, even though the reset screen itself (`pages/reset-password.html`) is already fully built. Reported 2026-08-14 by hitting the real emailed link, not yet root-caused — a quick server-side check (curling `/pages/reset-password.html?token=...` directly, no session cookie) returned the page correctly with no redirect, so the cause isn't as simple as the route being behind the login-required middleware; needs checking what the actual link in the email resolves to and what happens on click, not just the route in isolation. Likely just a matter of correcting a redirect somewhere per user's report, not a missing feature.
