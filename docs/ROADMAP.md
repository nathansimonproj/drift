...
**Shipped (2026-09-11).** Reset-password link redirected to the sign-in page instead of the reset screen, even though `pages/reset-password.html` was already fully built — root-caused to `server/mailer.js` concatenating `APP_URL` directly onto `/pages/reset-password.html`; a trailing slash on `APP_URL` (as configured on Render) produced a double-slash path that Express doesn't match against the registered route, so it fell through to the `requireAuth` catch-all and redirected to login. Reproduced locally with a deliberately trailing-slashed `APP_URL` (confirmed `302` pre-fix vs. `200` post-fix on the exact link), fixed by stripping trailing slashes from `APP_URL` before building the link, then verified end-to-end against production (Neon + real SendGrid send) with a throwaway account: email arrived, reset succeeded, new password logs in, old password is rejected.
